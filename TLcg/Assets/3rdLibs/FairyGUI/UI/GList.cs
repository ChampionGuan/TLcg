		...
		override protected internal void GetSnappingPosition(ref float xValue, ref float yValue)
		{
			if (_virtual)
			{
				...
				else if (_layout == ListLayoutType.SingleRow || _layout == ListLayoutType.FlowVertical)
				{
					...
					float snaptoleft = 0.5f;
					float snaptoright = 0.5f;					
					if (scrollPane != null)
					{
						ismovingleft = scrollPane._ismovingleft;
						snaptoleft = scrollPane.snapToItemLeft;
						snaptoright = scrollPane.snapToItemRight;
					}
					float saved = xValue;
					int index = GetIndexOnPos2(ref xValue, false);
					if (index < _virtualItems.Count && index < _realNumItems &&
						( (ismovingleft && saved - xValue > _virtualItems[index].size.x * snaptoleft) || 
						 (!ismovingleft && xValue + _virtualItems[index].size.x - saved < _virtualItems[index].size.x * snaptoright) ) ) 
