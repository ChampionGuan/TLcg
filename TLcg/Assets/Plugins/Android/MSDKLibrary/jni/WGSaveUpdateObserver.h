...
class WGSaveUpdateObserver : public WGBaseObserver
{
   public:
    /**
     * 检查应用宝后台是否有更新包的回调, 如果有更新包，则返回新包大小、增量包大小,
     * 游戏结合此接口返回结果和自身业务场景确定是否弹窗提示用户更新
    ...
    virtual void OnCheckNeedUpdateInfo(long newApkSize, std::string newFeature, long patchSize, int status,
        std::string updateDownloadUrl, int updateMethod) = 0;
     ...
     * @param state 下载状态 TMAssistantDownloadSDKTaskState(WGPublicDefine.h中有定义)(省流量更新，跳应用宝进行自更新)
     * 或 TMSelfUpdateSDKTaskState(WGPublicDefine.h中有定义)(使用sdk自更新)
     * @param errorCode TMAssistantDownloadSDKErrorCode(WGPublicDefine.h中有定义)(省流量更新，跳应用宝进行自更新) 或
     * TMSelfUpdateSDKErrorCode(WGPublicDefine.h中有定义)(使用sdk自更新) 错误码
     * @param errorMsg 错误信息
     */
...
public:
    WGSaveUpdateObserver *save_update_observer_;
};
