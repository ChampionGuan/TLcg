    ...
    local t = {
        ...
        Update = function(self)
            ...
            if nil ~= self.nextAuidoGroupId and not self.audioSource.isPlaying then
                Audio.PlaySound(self.nextAuidoGroupId, nil, AudioStat.Play, self.theFollow)
                self.nextAuidoGroupId = nil
            end
...
-- 获取音乐开关
function Audio.GetSwitchAudioMusic()
    return PlayerPrefs.GetAudioMusicSwitch()
end

-- 全局声效
function Audio.GetAudioEffectVolume()
    return PlayerPrefs.GetAudioEffectVolume()
end

-- 全局音乐
function Audio.GetAudioMusicVolume()
    return PlayerPrefs.GetAudioMusicVolume()
end

-- 全局声效开关
function Audio.SwitchAudioEffect(on)
...
-- 全局音乐开关
function Audio.SwitchAudioMusic(on)
    audioMusicSwitch = on
...
-- 全局声效
function Audio.SetAudioEffectVolume(volume, save)
...
-- 全局音乐
function Audio.SetAudioMusicVolume(volume, save)
    if nil == save or save then
...
-- 播放组audio
function Audio.PlayGroupSound(groupName, state)
...
-- 播放audio
function Audio.PlaySound(id, insId, state, followTarget)
    local asData = nil
...
function Audio.Initialize()
    ...
    m_musicAudio = {}
    m_effectAudio = {}
    audioMusicSwitch = Audio.GetSwitchAudioMusic()
    audioEffectSwitch = Audio.GetSwitchAudioEffect()

    -- 设置全局音量
    Audio.SetAudioEffectVolume(Audio.GetAudioEffectVolume())
    Audio.SetAudioMusicVolume(Audio.GetAudioMusicVolume())
end
